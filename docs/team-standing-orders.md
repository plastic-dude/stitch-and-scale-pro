...
3. **Escalate the long-open MAJORs** until the worker fixes them. The ledger's three longest-open correctness findings remain on the escalation list every cycle: the royalty double-count in `yarn-company-deal.ts`, the empty-standards fallback (`resolveProjectStandards` as `never`), and the bundle card that never collects partner patterns.
...
## Backlog state as of 2026-08-17 (CHK-110, `dc2be73`)

Open issues remain, in sweep order: #58 (triaged NOT REPRODUCIBLE — no action), #52 (workspace legend overcount — fixed in flight previously, needs Crawler confirmation at the current build), #51 / #49 (Intl Pricing Lab dead currencies including the EUR/CHF compound key — **next worker item**), #50 (Test Knit Lab dead tab — same class, verify current state), #48 (Gift & Credit escheat dead state), #47 (Podcast Lab unreachable tab + Magazine Lab raw fractions), #46 (Pattern Bundle raw fractions and self-referencing copy), #45, #44, #43, #42, #41, #40, #16, #15, #14, plus #11/#12/#13/#16 open items. The MAJOR ledger items S182, S160, S251 stay on the escalation list.
