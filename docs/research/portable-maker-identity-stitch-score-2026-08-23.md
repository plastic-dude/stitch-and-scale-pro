...
2. **Two independent research passes, on two separate scheduled firings**, before any implementation ticket opens. Each pass re-derives conclusions from live code at current HEAD, not from this document's cached assumptions.
   - **Pass 1:** re-verify §5 (current-state audit) against live HEAD, and confirm the plain "self-reported" export approach in §6.2 is sound. §6.2's cryptographic options are already closed by owner decision (2026-08-23) — kept simple and label-honest rather than key-based, specifically because part of this app's userbase is elderly and non-technical and should never need to think about cryptography. Pass 1 does not need to evaluate those options.
...
This maps almost exactly onto Stitch & Scale's situation, and was the basis for a device-local-keypair adaptation this brief originally floated for Pass 1 to investigate. **That direction is closed by owner decision (§6.2, 2026-08-23):** this app's userbase includes elderly, non-technical makers, and no export mechanism here should require them to understand keys or signing. This section is retained as grounding for *why* naive local export has a real honesty gap (§4 explains the same for the existing fingerprint) — not as a recommendation to build key-based verification. The chosen answer is the plain, clearly-labeled "self-reported" export in §6.2.
...
### 6.2 — The verifiability problem — **decided: keep it simple, no cryptography**

**Owner decision, 2026-08-23: option 1 only.** Export the data plainly, clearly labeled "self-reported," and let any future community site decide how (or whether) to weight self-attested data. The device-local-signing-keypair and full Open Badges 3.0 / Verifiable Credentials options (previously listed here as escalating candidates) are **explicitly ruled out** — not because they're technically wrong, but because they add real complexity (key management, a verifier a future site would need to implement) for a userbase that includes elderly, non-technical makers who need this to stay easy, not become a cryptography lesson. Pass 1 does not need to evaluate or size options 2/3 — they are closed. Pass 1 should instead confirm that a plain, clearly-labeled "self-reported" export is honest-by-construction (the label itself is the honesty mechanism: nobody receiving the data is misled into thinking it's independently verified) and note anything that would need to change in `recognition.ts`'s conventions to support it.
