...
The exact code-bearing Vercel deployment for this correction is `dpl_144CvR6kGa5bDZ6HAoBE3JuExMAu`. It reached `READY`, targets `production`, carries commit SHA `9f00918762a6168b0fb18c05e86047c84c3839c6`, and served the active public alias [`stitch-and-scale-pro-api-server.vercel.app`](https://stitch-and-scale-pro-api-server.vercel.app). No manual alias assignment was used. After this verification note and ledger row were pushed to `main`, Vercel automatically built documentation commit `720046c6060e504382162b8cca6f1c29547a3763` as production deployment `dpl_BPt4upgvEoo3UQoM71tMuwCyRtFE`; it reached `READY`. The only source diff from `9f00918` to that commit is the documentation checkpoint, so this later deployment changes release metadata but not application code. Fresh active-alias route, MCP-origin, and Project Book smoke checks passed after it became ready. It must not be described as a new application-code correction or as a manually assigned alias.
