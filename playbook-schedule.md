...
- [CHK-096] Removed unauthorized CHK-095 shell footer additions (EMLUX attribution and copy-link affordance) and fixed the public `/landing` route rendering the app shell and its NotFound fallback beneath the landing page. Verified typecheck, 1,694 tests / 88 files, build, and browser rendering on port 5000.
- [CHK-095] [2026-08-15] Vision-doc gap audit (user's old STITCH&SCALESURVEDOCS.md). Cross-checked every line: Grading Engine 100%, Settings ~80%, PDF system ~95%, workflow ~90%, Custom 404/onboarding/icons done. PWA was 20% in the doc but code already ships manifest.json + sw.js (sns-shell-v1) + SW registration + install-banner.tsx/use-pwa-install + apple meta -> ~95%. Dark mode (light/dark/system) already wired in SettingsContext + settings.tsx. ONLY real gaps: parent branding (footer now 'A premium tool for independent knitwear designers - by Emlux' + shareable 'Copy this tool's link' button), cloud sync (pending Supabase anon/service_role keys), permanent account deletion (post-auth), donations (waits for founder's payment platform choice - no placeholder href added). gates: typecheck clean, vitest 1,694/88, build green. Footer screenshot: localhost landing.
- [CHK-094] [2026-08-15] Designer-grade shareables redesign + bundle split (per user mandate + Claude's review: 2.1MB bundle). Research session 85 = bad-design failure patterns (clutter, template look, accent overuse, tautological copy) and best-in-class craft/infographic aesthetics; built inverse rules D-1..D-10. (a) Brag Cards rebuilt with 6 unique styles: Navy, Editorial, Gauge Swatch, Selvedge, Swiss Poster, Stitch Cameo - knit textures, grid discipline, tabular mono numbers, <=3-hue palettes, STITCH & SCALE attribution replacing watermark, streak tautology fixed. (b) Receipt Lab gains 3 share styles: Chat / Craft Paper / Selvedge themed card. (c) PDF renderer: retired text-wall watermark, single faint rotated selvedge band (D-7). (d) Code-splitting: all 78 heavy lab cards in TabPanel converted to React.lazy + Suspense (named-export remap cardLazy), main chunk 2,132kB -> 778kB gzipped 239kB. (e) Stale doc counts synced: brand-voice-brief.md + founding-tester-posts.md 78->79 labs, landing 1,600+ -> 1,694+ tests. Browser-verified: Brag Cards (6 styles + accents), Receipt Lab (3 themes), Payback Lab all render lazily with no crash. gates: typecheck clean, vitest 1,694/88, build green. Commits e7027f3 (brag redesign) + eab113e (receipt/PDF/split/counts).
- [CHK-093] [2026-08-15] Payback Lab (79th tab, Pricing & Income group). Fresh competitor research (session 84, finance/admin gap) found Wave, Zoho, QuickBooks, Xero, Craftybase, Ardent Seller, and Yarny all track income/expenses but NONE compute when a design pays back its costs AND time (Woolly Wormhead: £130/pattern true cost, 49 copies costs-only break-even, 133 with time). Engine lib/payback-lab.ts: direct costs + overhead share across published designs + hours x hourly rate (floor 12, editable) = investment; live sales from Receipt Lab matched by name (substring); recoup copies, cost-only copies, deficit/surplus, months-since-last-sale bleed detection (>=3 months + deficit), what-if repricing +10%/+20%, All/Winners/Recouping tabs, paidBack requires real skin in the game (investment>0 or sales>0). Reads Design Ledger + Receipt Lab via projectStorage seam (read-only); writes only own settings key (hours, rate). 14 engine tests. 093b polish: fixed paidBack free-design loophole + studio stat counts relevant patterns. gates: typecheck clean, vitest 1,693/88, build green. Counts 78->79 (landing, count-drift, tab-registry guards, brand-voice-brief, founding-tester-posts).
- [CHK-092] [STITCH-AND-SCALE-PRO] Brag Cards runtime-crash fix — the newly shipped `bragcard` tab contained a CommonJS `require("@/lib/receipt-lab")` call inside the BragCardPreview function body, which Vite cannot bundle and which crashed the whole app to a blank page on tab click. Replaced with a top-level ESM import of `fmtMoney` from receipt-lab; removed the leftover unused variable. Verified end-to-end in the browser on the demo project: card renders with studio-name override, 4 highlight templates, 4 accent picks, live 1080x1080 preview, caption, copy/share/download buttons. Gates: typecheck clean, vitest 1,679 tests / 87 files, build green. Pushed 74a50bc. Standing rule added to the brain: never use `require()` inside Vite/ESM modules — top-level ESM imports only.
