...
**Screenshots are mandatory (stakeholder requirement).** For every key interaction (default state, each input/slider/toggle change, each new/changed view, each finding): capture the view BEFORE and AFTER the change as two distinct screenshots — never reuse the same file. Convert webp to PNG before committing, give descriptive filenames (e.g. `cycle4-03-mix-sales-input-test.png`), commit them alongside the report under `qa-shots-cycleN/` on the same qa branch, and embed them in the report with Markdown image syntax (`![caption](qa-shots-cycleN/...)`). Never push a report that has no screenshots.

### 5. Re-verify baseline after the session (still no code changes).

### 6. Deliver findings
Only if new findings exist:
- Append/rewrite `/home/ubuntu/qa-task/QA_REPORT.md` style report into `qa-report-<YYYY-MM-DD>.md`, commit on a new branch `qa/manus-<YYYY-MM-DD>`, push the branch ONLY. `main` must never receive QA commits. The report MUST embed screenshots (see step 4).
