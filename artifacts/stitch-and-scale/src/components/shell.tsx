    ...
    <div className="min-h-[100dvh] min-w-0 flex flex-col bg-background text-foreground transition-colors duration-200">
      ...
      <header className="sticky top-0 z-30 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-1 sm:gap-4">
            {/* CHK-129: below the desktop breakpoint labels hide (hidden lg:inline) and the
            ...
            <Link href="/" aria-label={projectsLabel} className={`min-h-11 min-w-11 p-2 rounded-md hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2 ${location === '/' ? 'bg-secondary/50' : ''}`}>
              ...
              <span className="hidden lg:inline text-sm font-medium">{projectsLabel}</span>
            </Link>
            <Link href="/portfolio" aria-label={t('nav.portfolio')} className={`min-h-11 min-w-11 p-2 rounded-md hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2 ${location === '/portfolio' ? 'bg-secondary/50' : ''}`}>
              ...
              <span className="hidden lg:inline text-sm font-medium">{t('nav.portfolio')}</span>
            </Link>
            <Link href="/settings" aria-label={settingsLabel} className={`min-h-11 min-w-11 p-2 rounded-md hover:bg-accent hover:text-accent-foreground transition-colors flex items-center gap-2 ${location === '/settings' ? 'bg-secondary/50' : ''}`}>
              ...
              <span className="hidden lg:inline text-sm font-medium">{settingsLabel}</span>
            </Link>
            
            <div className="w-px h-6 bg-border mx-2"></div>
            
            {/* CHK-129: below the desktop breakpoint this is icon-only (label hidden <sm) —
      ...
      <main className="flex-1 flex flex-col min-w-0 w-full max-w-7xl mx-auto p-4 pb-24 sm:p-6 sm:pb-24 md:p-8 md:pb-24 lg:pb-8">
        <div key={location} className="flex-1 flex flex-col min-w-0 route-transition">
          {children}
        </div>
      </main>

      <nav aria-label="Primary mobile navigation" className="lg:hidden fixed left-0 right-auto bottom-0 z-40 w-[100dvw] max-w-[100dvw] box-border border-t border-border/70 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 pb-[env(safe-area-inset-bottom)]">
