...
const de: TechEditCopy = {
  ...
  findingGa05Detail: (r) => `Ein Rapport von ${r} rundet auf die nächste ganze Zahl wie ohne Rapport. Setze den tatsächlichen Rapport des Musters (z. B. 6 für einen 6-Maschen-Zopf) oder lösche ihn.`,
  findingGa05bTitle: 'Maschenrest ist ungültig für den Rapport',
  findingGa05bDetail: (rem, rep) => `Der Rest ${rem} liegt außerhalb des gültigen Bereichs 0…${rep - 1} für einen Rapport von ${rep}. Gültige Maschenzahlen wären …${rep + rem}, ${2 * rep + rem}, …`,
  findingGa05cTitle: 'Sowohl Parität als auch Rapport gesetzt — Parität gewinnt',
  findingGa05cDetail: 'Wenn beides gesetzt ist, hat die Paritätsrundung Vorrang. Behalte die Einschränkung, die dieses Maß tatsächlich steuert.',
  findingGa06Title: (s) => `Null oder negative Maschenzahl bei Größe ${s}`,
  findingGa06Detail: (c) => `Die gerundete Maschenzahl ist ${c}. Ein so kleiner Wert bedeutet meist, dass Basiswert, Maschenprobe oder Gradierungsschlüssel für dieses Maß falsch sind.`,
  findingGa07Title: 'Längenschlüssel als Breite gradiert',
  findingGa07Detail: (l, k) => `${l} verwendet den Schlüssel ${k} (eine Länge) mit dem Typ "Breite", was den Wert beim Gradieren halbiert. Wenn dieses Maß eine Länge sein soll, setze den Typ auf "Länge" oder "direkt".`,
  ...
  findingGa08bDetail: 'Nur eine Größe ist gradiert. Einzelgrößen-Muster sind ein häufiger Beschwerdegrund bei Käufern ("Warum gibt es das nicht in mehr Größen?") — Mehrgrößen-Muster verkaufen sich auf Ravelry und Etsy konsistent besser. Erwäge, mindestens 3 Größen zu gradieren, bevor du veröffentlichst.',
  findingGa09Title: (p, std, s) => `Basiswert ${p}% unter dem ${std} ${s} Standard`,
  findingGa09Detail: (v, u, std, s, t) => `Du hast ${v}${u} für dieses Maß eingegeben, aber der ${std} Standard-Körperwert für Größe ${s} ist ${t}${u}. Ein Kleidungsstück kann nicht kleiner sein als der Körper, den es bedeckt — prüfe, ob dies ein Umfangs-Gradierungsschlüssel sein sollte oder ob der Basiswert zu einer anderen Größe gehört.`,
  findingGa09bTitle: (std, s) => `Basiswert mehr als das Doppelte des ${std} ${s} Standards`,
  findingGa09bDetail: (v, u, t) => `Du hast ${v}${u} eingegeben vs. einem Körperwert von ${t}${u}. Bequemlichkeitszugabe erklärt Abweichungen, aber keine Verdoppelung — dies bedeutet meist, dass ein Umfang eingegeben wurde, wo eine Halbreite hingehört (oder umgekehrt).`,
...
const fr: TechEditCopy = {
  ...
  findingGa01Detail: 'Définissez les mailles/rangs pour 10 cm dans l\'échantillon du projet. Un éditeur technique ne peut pas vérifier les calculs sans cela, et un testeur non plus. C\'est le premier point de toute liste de vérification pré-audit.',
  ...
  findingGa02bDetail: 'Une valeur de base nulle exclut cette mesure de toutes les tailles gradées. Si c\'est intentionnel (ex. un panneau décoratif), c\'est correct ; sinon, saisissez la valeur de la taille de base.',
  findingGa03Title: 'La progression des tailles n\'est pas monotone',
  findingGa03Detail: (s, v, u) => `Les valeurs physiques pour ${s} sont : ${v} ${u}. Les dimensions du vêtement devraient croître (ou rester stables) avec la taille — vérifiez si la clé de gradation ou le type de cette mesure est correct.`,
  findingGa04Title: (p, s) => `Nombre de mailles décalé de ${p}% par rapport à la cible brute (${s})`,
  findingGa04Detail: (r, raw) => `Arrondi à ${r} mailles vs ${raw} brut. La contrainte de rapport/parité du motif force le compte loin de la mesure cible — vérifiez le rendu final à cette taille.`,
  findingGa04bTitle: (p, s) => `Nombre de rangs décalé de ${p}% par rapport à la cible brute (${s})`,
  findingGa04bDetail: (r, raw) => `Arrondi à ${r} rangs vs ${raw} brut. La tolérance sur la longueur est généralement souple, mais signalez-le si la mesure est critique (ex. profondeur d'emmanchure).`,
  findingGa05Title: (r) => `Un rapport de mailles de ${r} est sans effet`,
  findingGa05Detail: (r) => `Un rapport de ${r} arrondit à l'entier le plus proche comme s'il n'y avait pas de rapport. Définissez le multiple réel du motif (ex. 6 pour une torsade de 6 mailles) ou effacez-le.`,
  findingGa05bTitle: 'Le reste des mailles est invalide pour le rapport',
  findingGa05bDetail: (rem, rep) => `Le reste ${rem} est en dehors de la plage valide 0…${rep - 1} pour un rapport de ${rep}. Les comptes valides seraient …${rep + rem}, ${2 * rep + rem}, …`,
  findingGa05cTitle: 'Parité et rapport définis — la parité l\'emporte',
  findingGa05cDetail: 'Lorsque les deux sont définis, l\'arrondi par parité prime. Gardez la contrainte qui régit réellement cette mesure.',
  ...
  findingGa08bTitle: 'Modèle à taille unique',
  findingGa08bDetail: 'Une seule taille est gradée. Les modèles à taille unique sont une source fréquente de plaintes d\'acheteurs ("pourquoi n\'y a-t-il pas plus de tailles ?") — les modèles multi-tailles se vendent systématiquement mieux sur Ravelry et Etsy. Envisagez de grader au moins 3 tailles avant de publier.',
  findingGa09Title: (p, std, s) => `Valeur de base ${p}% en dessous du standard ${std} ${s}`,
  findingGa09Detail: (v, u, std, s, t) => `Vous avez saisi ${v}${u} pour cette mesure, but la valeur corporelle standard ${std} pour la taille ${s} est ${t}${u}. Un vêtement ne peut pas être plus petit que le corps qu'il couvre — vérifiez s'il doit s'agir d'une clé de gradation de circonférence, ou si la valeur de base appartient à une autre taille.`,
  findingGa09bTitle: (std, s) => `Valeur de base plus du double du standard ${std} ${s}`,
  findingGa09bDetail: (v, u, t) => `Vous avez saisi ${v}${u} vs une valeur corporelle de ${t}${u}. L'aisance explique l'écart, mais pas un doublement — cela signifie généralement qu'une circonférence a été saisie là où une demi-largeur devrait être (ou vice-versa).`,
  findingGa10Title: (l) => `Libellé de mesure en double « ${l} »`,
  findingGa10Detail: (n, s) => `Ce libellé apparaît ${n} fois dans « ${s} ». Deux mesures avec le même nom perturbent à la fois les testeurs et les éditeurs — donnez à chacune un nom distinct (ex. « Buste (devant) » / « Buste (dos) »).`,
...
const es: TechEditCopy = {
  ...
  findingGa04bDetail: (r, raw) => `Redondeado a ${r} vueltas vs ${raw} brutas. La tolerancia del largo suele ser flexible, pero avise si la medida es crítica (p. ej., profundidad de la sisa).`,
