...
import { RELEASE_TIMING_COPY } from '../release-timing-copy';

describe('release-timing-copy (QUEUE-003 localization module)', () => {
  const locales = ['en', 'de', 'fr', 'es', 'pt'] as const;

  it('exports all 5 locales', () => {
    for (const loc of locales) {
      expect(RELEASE_TIMING_COPY[loc]).toBeDefined();
    }
    expect(Object.keys(RELEASE_TIMING_COPY).length).toBe(5);
  });

  it('has no empty values in any locale', () => {
    for (const loc of locales) {
      const c = RELEASE_TIMING_COPY[loc];
  ...
  it('has identical key sets across all locales', () => {
    const enKeys = Object.keys(RELEASE_TIMING_COPY.en).sort();
    for (const loc of locales) {
      expect(Object.keys(RELEASE_TIMING_COPY[loc]).sort()).toEqual(enKeys);
    }
  });

  it('de strings are not English leftovers', () => {
    const de = RELEASE_TIMING_COPY.de;
    const en = RELEASE_TIMING_COPY.en;
  ...
  it('non-en strings differ from English where localized', () => {
    for (const loc of ['de', 'fr', 'es', 'pt'] as const) {
      const locCopy = RELEASE_TIMING_COPY[loc];
      const en = RELEASE_TIMING_COPY.en;
      let diffs = 0;
      for (const k of Object.keys(en) as (keyof typeof en)[]) {
  ...
  it('spans all interface keys of ReleaseTimingCopy', () => {
    const en = RELEASE_TIMING_COPY.en;
    // module keys cover the interface contract (no missing values)
    expect(Object.keys(en).length).toBeGreaterThan(0);
    expect(en['baselineSalesMonthFlat']).toBe('Baseline sales / month (flat season)');
  });
  ...
  it('falls back to English semantics via spread in locale consts', () => {
    ...
    const de = RELEASE_TIMING_COPY.de;
    expect(de['baselineSalesMonthFlat']).toBeDefined();
    expect(typeof de['baselineSalesMonthFlat']).toBe('string');
  });
});
