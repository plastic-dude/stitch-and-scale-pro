import { describe, expect, it } from 'vitest';
import { PRICING_PSYCHOLOGY_COPY } from '../pricing-psychology-copy';

describe('pricing-psychology-copy (QUEUE-003 localization module)', () => {
  const locales = ['en', 'de', 'fr', 'es', 'pt'] as const;

  it('exports all 5 locales', () => {
    for (const loc of locales) {
      expect(PRICING_PSYCHOLOGY_COPY[loc]).toBeDefined();
    }
    expect(Object.keys(PRICING_PSYCHOLOGY_COPY).length).toBe(5);
  });

  it('has no empty values in any locale', () => {
    for (const loc of locales) {
      const c = PRICING_PSYCHOLOGY_COPY[loc];
      for (const k of Object.keys(c) as (keyof typeof c)[]) {
        const v = c[k];
        if (typeof v === 'string') {
          expect(v.length).toBeGreaterThan(0);
        } else {
          expect(v).toBeDefined();
        }
      }
    }
  });

  it('has identical key sets across all locales', () => {
    const enKeys = Object.keys(PRICING_PSYCHOLOGY_COPY.en).sort();
    for (const loc of locales) {
      expect(Object.keys(PRICING_PSYCHOLOGY_COPY[loc]).sort()).toEqual(enKeys);
    }
  });

  it('de strings are not English leftovers', () => {
    const de = PRICING_PSYCHOLOGY_COPY.de;
    const en = PRICING_PSYCHOLOGY_COPY.en;
    let diffs = 0;
    const keys = Object.keys(en) as (keyof typeof en)[];
    for (const k of keys) {
      if (de[k] !== en[k]) diffs++;
    }
    // Most strings should differ, but some technical terms (e.g. "Mainstream", "Premium") are shared
    expect(diffs / keys.length).toBeGreaterThan(0.7);
  });

  it('non-en strings differ from English where localized', () => {
    for (const loc of ['de', 'fr', 'es', 'pt'] as const) {
      const locCopy = PRICING_PSYCHOLOGY_COPY[loc];
      const en = PRICING_PSYCHOLOGY_COPY.en;
      let diffs = 0;
      for (const k of Object.keys(en) as (keyof typeof en)[]) {
        if (locCopy[k] !== en[k]) diffs++;
      }
      // at least 80% of entries must differ from English
      expect(diffs / Object.keys(en).length).toBeGreaterThan(0.8);
    }
  });

  it('spans all interface keys of PricingPsychologyCopy', () => {
    const en = PRICING_PSYCHOLOGY_COPY.en;
    // module keys cover the interface contract (no missing values)
    expect(Object.keys(en).length).toBeGreaterThan(0);
    expect(en['bundleCandidateTotal']).toBe('Bundle candidate total');
  });

  it('falls back to English semantics via spread in locale consts', () => {
    // de/fr/es/pt consts are built with {...en, ...} so any key absent from a
    // locale map still resolves to the English string.
    const de = PRICING_PSYCHOLOGY_COPY.de;
    expect(de['bundleCandidateTotal']).toBeDefined();
    expect(typeof de['bundleCandidateTotal']).toBe('string');
  });
});
