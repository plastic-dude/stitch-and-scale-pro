...
export function buildGradingCsv(results: GradingResult, unit: MeasurementUnit = 'in'): string {
  const lines = [`Section,Measurement,Property,${ALL_SIZES.join(',')}`];
  ...
  for (const section of results) {
    for (const measurement of section.measurements) {
      ...
      lines.push(`${prefix},Stitches,${stitches}`);
      ...
      lines.push(`${prefix},Physical (${unit}),${physical}`);
    }
  }
